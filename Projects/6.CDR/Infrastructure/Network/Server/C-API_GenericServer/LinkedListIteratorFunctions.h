/**
 * @file LinkedListIteratorFunctions.h
 * @author Natan Meirov (NatanMeirov@gmail.com)
 * @brief Adds additional Iterator functions to support the LinkedListIterator
 * @version 1.1
 * @date 2021-03-24
 * 
 * @copyright Copyright (c) 2021
 * 
 */


#ifndef __LINKEDLISTITERATORFUNCTIONS_H__
#define __LINKEDLISTITERATORFUNCTIONS_H__

/* Includes: */

#include <stddef.h> /* size_t */
#include "LinkedListIterator.h"


/* Defines: */

/** 
 * @brief Predicate function returns a none zero value if predicate holds for element
 *
 * @param[in] _element : element to test
 * @param[in] _context : context to be used
 * @return none zero if predicate holds
 */
typedef int (*PredicateFunction)(void* _element, void* _context);


/** 
 * @brief Action function to operate on an element
 *
 * @param[in] _element : element to test
 * @param[in] _context : context to be used
 * @return zero if action fails
 */
typedef int (*ListActionFunction)(void* _element, void* _context);


/** 
 * @brief Action function to check if a < b
 *
 * @param[in] _a : element to test
 * @param[in] _b : element to test against
 * @return none zero if _a < _b
 */
typedef int (*LessFunction)(void* _a, void* _b);


/** 
 * @brief Action function to check if a == b
 *
 * @param[in] _a : element to test
 * @param[in] _b : element to test against
 * @return none zero if _a == _b
 */
typedef int (*EqualsFunction)(void* _a, void* _b);


/*---------------------------------------------- Main API Functions -----------------------------------------------*/

/** 
 * @brief Finds the first element in a range satisfying a predicate
 * @details find the element for which the predicate returns a non zero value
 *  in the half open range [begin..end)
 *
 * @param[in] _begin : start search from here
 * @param[in] _end : end search here - not included
 * @param[in] _predicate : Predicate function
 * @param[in] _context : context to be passed to the predicate
 * @return an iterator pointing to the first node with data found or to _end if not
 */
LinkedListIterator LinkedListIteratorFindFirst(LinkedListIterator _begin, LinkedListIterator _end, PredicateFunction _predicate, void* _context);


/** 
 * @brief count elements in a sub list satisfying predicate
 * @details find the element for which the predicate returns a non zero value
 *  in the half open range [begin..end)
 *
 * @param[in] _begin : start search from here
 * @param[in] _end : end search here - not included
 * @param[in] _predicate : Predicate function
 * @param[in] _context : context to be passed to the predicate
 * @return count of all elements where _predicate(e, _context) != 0, or MAX_SIZE_T if error has occurred
 */
size_t LinkedListIteratorCountIf(LinkedListIterator _begin, LinkedListIterator _end, PredicateFunction _predicate, void* _context);


/** 
 * @brief execute an action for all elements in a half open range
 * @details call -action for each element in the range [begin..end)
 * iteration will stop if Action function returns 0 for an element
 *
 * @param[in] _begin: A list iterator to start operations from
 * @param[in] _end : A list iterator to end operations on
 * @param[in] _Action : user provided action function
 * @param[in] _context : parameters for the function
 * @return LinkedListIterator to the element where the iteration stoped. this might be end iterator
 */
LinkedListIterator LinkedListIteratorForEach(LinkedListIterator _begin, LinkedListIterator _end, ListActionFunction _action, void* _context);


/** 
 * @brief Sorts a sublist in place using the bubble sort algorithm
 * Complexity O(?)
 *
 * @param[in] _begin : Iterator to sublist start
 * @param[in] _end : Iterator to sublist end
 * @param[in] _less : less compare function
 * 
 */
void LinkedListIteratorSort(LinkedListIterator _begin, LinkedListIterator _end, LessFunction _less);


/** 
 * @brief Splice all element from [_begin.._end) into _dest
 * @details remove all elements from the half open range [_begin.._end)
 * and insert them before _dest
 * O(?)
 *
 * @warning The function assumes that:
 *  _begin and _end are in the correct order and on the same list
 *  _dest is not between them
 * Otherwise behavior is undefined
 * all three iterators can belong to same list
 */
LinkedListIterator LinkedListIteratorSplice(LinkedListIterator _dest, LinkedListIterator _begin, LinkedListIterator _end);


/** 
 * @brief Merges two given sub lists into destination
 * @details Merge elements from two sub lists defined by [_firstbegin.._firstEnd)
 * and [_secondBegin.._secondEnd) in optionally sorted order using _less function.
 * merged elements will be inserted before _destBegin
 * if a _less function is provided then merge will be sorted
 * 
 * @warning this will removes all merged items from source ranges.
 * O(?)
 *  
 */
LinkedListIterator LinkedListIteratorMerge(LinkedListIterator _destBegin, LinkedListIterator _firstBegin, LinkedListIterator _firstEnd,
			LinkedListIterator _secondBegin, LinkedListIterator _secondEnd, LessFunction _less);


/** 
 * @brief Removes a sublist [_begin.._end) from a list and creates a new list
 * with all removed items.
 * O(?)
 *
 */
LinkedList* LinkedListIteratorCut(LinkedListIterator _begin, LinkedListIterator _end);


/** 
 * @brief Removes all duplicate elements from a sorted sublist [_begin.._end)
 * and return a new list with all removed items.
 * O(?)
 * post condition: [_begin.._end) contains only unique elements
 */
LinkedList* LinkedListIteratorUnique(LinkedListIterator _begin, LinkedListIterator _end, EqualsFunction _equals);

/*------------------------------------------- End of Main API Functions -------------------------------------------*/

#endif /* #ifndef __LINKEDLISTITERATORFUNCTIONS_H__ */
