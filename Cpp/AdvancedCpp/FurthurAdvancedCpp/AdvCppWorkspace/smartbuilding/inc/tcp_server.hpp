...
// Concept of ClientMessageHandler: should be a functor that implements: operator()(Message, ClientID, Response&) while Message is the received buffer, and ClientID is the client that sent that message, and a REFERENCE to a semi filled Response object to send back, while response is:
...
// Concept of NewClientConnectionHandler: should be a functor that implements: operator()(ClientInfo, Response&)- while client info is the information about the new connected client, and a REFERENCE to a Response object (like above)
...
private:
    ...
    std::list<ClientID> m_connectedClients;
    std::unordered_map<ClientID,ClientInfo> m_connectedClientsInfoTable;
