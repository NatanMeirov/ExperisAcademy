 ...
 * @param[out] _pKey: pointer to variable to return the key from HashMap by reference (if key was found), cannot be NULL
 * @param[out] _pValue: pointer to variable to return the value from HashMap by reference, cannot be NULL
 * @return HashMapResult - success or error status code
 * @retval HASHMAP_SUCCESS on success
 * @retval HASHMAP_UNINITIALIZED_ERROR on error - given pointer is NULL
