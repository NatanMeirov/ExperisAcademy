...
struct Server
{
    ...
    int m_currentConnectedClientsCount;
    int m_maxAmountOfConnectedClientsAtSameTime;
};
...
Server* ServerCreate(void* _applicationInfo, ClientMessageHandler _onClientMessage, ErrorHandler _onError, NewClientConnectionHandler _onNewClientConnection, CloseClientConnectionHandler _onCloseClientConnection, unsigned int _serverPort, int _maxWaitingConnections)
{
    ...
    if(_serverPort < MIN_PORT_VALUE || _serverPort > MAX_PORT_VALUE || _maxWaitingConnections <= 0)
    {
        return NULL;
    }
    ...
    while(iterator != endOfLinkedList)
    {
        ...
        if(FD_ISSET(*currentClientSocket, &_socketsSignalsIndicator))
        {
            ...
            else /* CLIENT_KEEP - message has successfully received for the client */
            {
                ...
                _server->m_onClientMessage((void*)receivedMessageBuffer, *currentClientSocket, &response, _server->m_applicationInfo);
