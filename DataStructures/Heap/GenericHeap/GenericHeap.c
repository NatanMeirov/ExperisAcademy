...
static HeapResult ValidateNotNullHeap(const Heap* _heap);
static HeapResult ValidateNotNullVector(const Vector* _vector);
static HeapResult ValidateNotNullItem(void* _item);
...
Heap* HeapBuild(Vector* _vector, LessThanComparator _ptrFuncLessThen)
{
    ...
    if(ValidateNotNullHeap(newHeap) == HEAP_NOT_INITIALIZED)
    {
        return NULL;
    }
...
Vector* HeapDestroy(Heap** _heap)
{
    ...
    if(!_heap || ValidateNotNullHeap(*_heap) != HEAP_SUCCESS || ValidateNotNullVector((*_heap)->m_vector) != HEAP_SUCCESS)
    {
        return NULL;
    }
...
{
    HeapResult statusCode;
    VectorResult vecStatusCode;

    if((statusCode = ValidateNotNullHeap(_heap)) != HEAP_SUCCESS || (statusCode = ValidateNotNullVector(_heap->m_vector)) != HEAP_SUCCESS
    || (statusCode = ValidateNotNullItem(_element)) != HEAP_SUCCESS /* Heap cannot include NULLs */)
    {
        return statusCode;
    }
...
const void* HeapPeekTop(const Heap* _heap)
{
    ...
    if(ValidateNotNullHeap(_heap) != HEAP_SUCCESS || ValidateNotNullVector(_heap->m_vector) != HEAP_SUCCESS)
    {
        return NULL;
    }
...
static HeapResult ValidateNotNullHeap(const Heap* _heap)
{
    return (!_heap) ? HEAP_NOT_INITIALIZED : HEAP_SUCCESS;
}


static HeapResult ValidateNotNullVector(const Vector* _vector)
{
    return (!_vector) ? HEAP_NOT_INITIALIZED : HEAP_SUCCESS;
}


static HeapResult ValidateNotNullItem(void* _item)
{
    return (!_item) ? HEAP_NOT_INITIALIZED : HEAP_SUCCESS;
}
