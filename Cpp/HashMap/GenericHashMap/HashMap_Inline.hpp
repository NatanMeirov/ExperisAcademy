...
HashMap::HashMap<K,V>::HashMap(const size_t a_initialSize, KeyHashingHandler<K>* a_hashHandler)
: m_buckets(a_initialSize) // Every list is initialized using default c'tor (a_initialSize lists)
...
typename HashMap::HashMap<K,V>::Iterator HashMap::HashMap<K,V>::Find(const K& a_key) const {
    ...
    return HashMapIterator<K,V>(currentBucketsIterator, const_cast<typename Types<K,V>::OuterContainer&>(this->m_buckets).end(), bucketIterator);
}
